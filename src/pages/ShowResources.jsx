...
export default function ShowResources() {
  ...
  // ── ADD this near the top of the component (after const navigate) ──
  const loggedInUser   = JSON.parse(localStorage.getItem("loggedInUser") || "{}");
  const userFullName   = loggedInUser.fullName   || "Faculty";
  ...
  const fetchResources = async (cat) => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/resources/category`, {
        params: {
          name:       cat,
          department: userRole === "HOD" ? null : userDepartment  // HOD sees all
        }
      });
      const list = res.data;
      setResources(list);
  ...
  const fetchTimetable = async (resourceId , day ) => {
  ...
  try {
    ...
    const [timetableRes, bookingsRes] = await Promise.all([
      ...
      axios.get(
        ...
        `${API_BASE}/resources/${resourceId}/timetable?day=${day}`
      ),
        ...
        <header className="rc-header">
          ...
          <div className="rc-header-right">
            ...
            <div className="rc-user-chip">
              <div className="rc-avatar">{userInitial}</div>
              <div className="rc-user-text">
                <span className="rc-user-name">{userFullName}</span>
                <span className="rc-user-role">{userRole}</span>
              </div>
              <ChevronDownIcon />
            </div>
          </div>
